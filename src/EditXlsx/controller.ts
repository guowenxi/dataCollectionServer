...
export class AppController {
  ...
  async importXlsx(
    ...
    const instance = await this.service.importXlsx(moduleId, files[0]);
    return {
      code: 200,
      success: 1,
      message: "读取成功",
  ...
  async exportXlsx(
    ...
    const buffer = await this.service.exportXlsx(list);
    const _path = this.configService.get("filePath");
    fs.writeFileSync(
      `${_path}/${fileName}.xlsx`,
      buffer
    );
    const file = createReadStream(join(process.cwd(), _path, `${fileName}.xlsx`), {
